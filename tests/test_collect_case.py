...
class TestBuildCase(unittest.TestCase):
    def test_correction_valido(self):
        ...
        self.assertEqual(case["room"], "Quarto 01")
        self.assertEqual(case["component"], "Parede")
        ...
        self.assertEqual(case["source_session"], "sessao-1")
    ...
    def test_case_type_invalido_falha(self):
        with self.assertRaises(ValueError):
            build_case(
                case_type="invalido", room="Sala", component="Piso", source_text="texto"
            )
    ...
    def test_room_vazio_falha(self):
        with self.assertRaises(ValueError):
            build_case(
                case_type="no_correction_needed",
                room="   ",
    ...
    def test_component_vazio_falha(self):
        with self.assertRaises(ValueError):
            build_case(
                case_type="no_correction_needed",
                room="Sala",
    ...
    def test_ids_sao_unicos(self):
        case_1 = build_case(
            case_type="no_correction_needed", room="Sala", component="Piso", source_text="a"
        )
        case_2 = build_case(
            case_type="no_correction_needed", room="Sala", component="Piso", source_text="a"
        )
...
class TestAppendCase(unittest.TestCase):
    def test_append_grava_jsonl_valido(self):
        with TemporaryDirectory() as tmp:
            ...
            case_2 = build_case(
                case_type="no_correction_needed", room="Sala", component="Piso", source_text="b"
            )
    ...
    def test_append_persiste_room_e_component(self):
        with TemporaryDirectory() as tmp:
            path = Path(tmp) / "corpus_real.jsonl"
            case = build_case(
                case_type="correction",
            ...
            append_case(case, path)

            saved = json.loads(path.read_text(encoding="utf-8").strip())
            self.assertEqual(saved["room"], "Quarto 01")
            self.assertEqual(saved["component"], "Parede")
